...
class gain_c:
    ...
    def __init__(self, instance_name, fs, signal_width=16, gain_int_width=4, gain_q_width=12, out_width=16):
        ...
        if (d == True):
            self.unit_test()
        self.generate()
        add_file_to_list("gain_c_" + self.instance_name + ".v")

    def generate(self):
        print("...generating Controlled Gain with name " + self.instance_name + "...")
        scale = 2**(self.gain_q_width - 1)
        with open(proj_name() + "gain_c_" + self.instance_name + ".v", 'w') as f:
            f.write("module gain_c_" + self.instance_name + "#\n")
            f.write("(\n")
            ...
            print("Done generating GAIN " + "gain_c_" + self.instance_name + ".v\n")
    ...
    def unit_test(self):
        ...
        with open(proj_name() + "gain_c_" + self.instance_name + ".do", 'w') as f:
            ...
            scal_val1 = int(1.5* (2**(self.gain_q_width - 1)))
            f.write("# Gain = 1.5 = 1.5* 2^" + str(self.gain_q_width - 1) +"= " + str(scal_val1) + "\n")
            f.write("force -freeze gain_in 10#" + str(scal_val1) + "  0\n")
            scal_val2 = int(2* (2**(self.gain_q_width - 1)))
            f.write("# Gain = 2 = 2* 2^" + str(self.gain_q_width - 1) +"= " + str(scal_val2) + "\n")
            f.write("force -freeze gain_in 10#" + str(scal_val2) + "  5000\n")
            f.write("\n")
